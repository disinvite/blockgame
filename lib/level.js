    ...
    if (shapeList.length > 0) {
      ...
      for (let row = this.dim - 1; row >= 0; row--) {
        ...
        for (let col = 0; col < this.dim; col++) {
          // any shapes in the bottom row aren't moving
          if (row === (this.dim - 1)) {
            if ( shapeList.includes( this.tiles[row][col] ) ) {
              notMovingList.push( this.tiles[row][col] );
              // don't break out here. we need to do the next check too
            }
          }
