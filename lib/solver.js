...
const { FastLevel } = require('./fastLevel');
/*
class Solution {
  constructor(level) {
...
function goDirection(levelStr, which, goalPosition) {
  ...
  level.goal.row = goalPosition.row;
  level.goal.col = goalPosition.col;
  if (which === 'up') {
    level.slideUp();
  } else if (which === 'down') {
    level.slideDown();
...
function buildGraph(level) {
  ...
  const fast = new FastLevel(level.dim, level.goal);
  ...
  while (queue.length > 0) {
    ...
    for (const dir of dirs) {
      ...
      const next = fast.move(current, dir);
...
function shortestPath(vertices, edges, graph, winningMoves) {
  ...
  for (const win of winningMoves) {
    ...
    while (queue.length > 0) {
      ...
      const theseAreNext = reverseEdges[current];
