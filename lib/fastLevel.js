...
function backToLevelString(grid, order) {
  const invert = order.map(list => list.reverse()).reverse();
  const tiles = grid.flat();
  return invert.map(list => {
    return list.map(i => tiles[i]).join('');
  }).join('\n');
}
...
class FastLevel {
  ...
  move(inputStr, direction) {
    ...
    */
    ...
    return backToLevelString(order, this.orderList[direction]);
  }
}
